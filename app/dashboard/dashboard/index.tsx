...
export default function DashboardOverviewPage() {
  ...
  const { data: summary } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get("/api/artworks/analytics/summary");
  ...
  return (
    <ScrollView className="flex-1 bg-stone-50 p-4" showsVerticalScrollIndicator={false}>
      ...
      <Link href="/dashboard/dashboard/analytics" asChild>
        <TouchableOpacity className="bg-white rounded-2xl p-5 border border-gallery-100 mb-6">
          <View className="flex-row items-center justify-between mb-3">
            <View>
              <Text className="font-semibold text-stone-800 text-lg">Analytics Overview</Text>
              <Text className="text-stone-500 text-sm">Quick performance snapshot</Text>
            </View>

            <View className="w-11 h-11 rounded-full bg-gallery-100 items-center justify-center">
          ...
          <View className="flex-row justify-between">
            <View className="items-center flex-1">
              ...
              <Text className="text-xs text-stone-500">Likes</Text>
            </View>
            ...
            <View className="items-center flex-1">
              ...
              <Text className="text-xs text-stone-500">Reviews</Text>
            </View>
            ...
            <View className="items-center flex-1">
              ...
              <Text className="text-xs text-stone-500">Followers</Text>
            </View>
          </View>

          <Text className="text-gallery-600 text-sm font-medium mt-4 text-center">
            View full analytics →
          </Text>
        </TouchableOpacity>
      </Link>
      ...
      <View className="gap-3 mb-6">
        <QuickAction
          ...
          desc="Add a new piece"
          primary
        />

        <QuickAction
          href="/dashboard/dashboard/artworks"
          icon="images-outline"
          label="Manage artworks"
          desc="View, edit, publish, or delete your artworks"
        />
        ...
        <QuickAction
          ...
          label="Analytics"
          desc="Track likes, reviews, followers, and sales"
        />
      ...
      {!isLoading && artworks && artworks.length > 0 && (
        <>
          <View className="flex-row justify-between items-center mb-3">
            <Text className="text-sm font-semibold text-stone-500 uppercase">
              Recent Artworks
            </Text>
    ...
    <Link href={href} asChild>
      <TouchableOpacity
        className={`bg-white rounded-xl p-4 border flex-row items-center ${
          primary ? "border-gallery-200" : "border-stone-100"
        }`}
      >
        <View
          className={`w-10 h-10 rounded-xl items-center justify-center mr-4 ${
            primary ? "bg-gradient-to-br from-gallery-500 to-purple-600" : "bg-stone-100"
          }`}
        >
