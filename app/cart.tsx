...
export default function CartPage() {
  ...
  const handleCheckout = () => {
    ...
    if (items.length === 0) {
      Alert.alert("Cart is empty", "Add some artworks before checkout.");
      return;
    }
  ...
  if (!isAuthenticated) {
    return (
      <View className="flex-1 bg-stone-50 justify-center items-center p-6">
        ...
        <Text className="font-display text-2xl font-bold text-stone-800 mt-4">
          Shopping Cart
        </Text>
        <Text className="text-stone-500 text-center mt-2 mb-6">
  ...
  return (
    <View className="flex-1 bg-stone-50">
      <ScrollView className="flex-1 p-4" showsVerticalScrollIndicator={false}>
        ...
        {items.map((item) => (
          <View key={item.artworkId} className="bg-white rounded-xl p-4 mb-3 shadow-card flex-row">
            <Image source={{ uri: item.imageUrl }} className="w-20 h-20 rounded-lg bg-stone-100" />
            <View className="flex-1 ml-3">
              <Link href={`/artwork/${item.artworkId}`} asChild>
                <TouchableOpacity>
                  <Text className="font-semibold text-stone-800" numberOfLines={1}>{item.title}</Text>
                </TouchableOpacity>
              </Link>
              <Text className="text-sm text-stone-500">{item.artistName}</Text>
              <Text className="font-medium text-stone-700 mt-1">{formatPrice(item.price)}</Text>
            </View>
            <View className="flex-row items-center gap-1">
              <TouchableOpacity
                onPress={() => updateQuantity(item.artworkId, item.quantity - 1)}
                className="p-2"
              >
                <Ionicons name="remove-outline" size={18} color="#78716c" />
              </TouchableOpacity>
              <Text className="w-6 text-center font-medium text-stone-700">{item.quantity}</Text>
              <TouchableOpacity
                onPress={() => updateQuantity(item.artworkId, item.quantity + 1)}
                className="p-2"
              >
              ...
              <TouchableOpacity
                onPress={() => removeItem(item.artworkId)}
                className="p-2 ml-1"
              >
      ...
      <View className="p-4 border-t border-stone-200 bg-white">
        <View className="flex-row justify-between mb-2">
          <Text className="text-stone-500">Subtotal ({totalItems} items)</Text>
          <Text className="font-medium text-stone-700">{formatPrice(totalPrice)}</Text>
        </View>
        ...
        <View className="flex-row justify-between mb-4">
          ...
          <Text className="text-xl font-bold text-gallery-600">{formatPrice(totalPrice)}</Text>
        </View>
